...
const WeatherCard: React.FC<WeatherCardProps> = ({ data, recentSearches, onRecentClick, onSeeAll }) => {
  ...
  return (
    <div className="flex flex-col w-full h-full justify-between">
      ...
      <div className="w-full flex-1 min-h-0 mt-2 overflow-hidden">
           <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData} margin={{ top: 10, right: 15, left: 15, bottom: 40 }}>
                 <defs>
                 ...
                 <XAxis 
                    ...
                    tickLine={false} 
                    orientation="top"
                    tick={{ fill: '#fff', fontSize: 14, fontWeight: 300, opacity: 0.9 }}
                    dy={-5}
                 />
                 ...
                 <XAxis 
                    ...
                    orientation="bottom"
                    tick={({ x, y, index }: any) => (
                       <text x={x} y={y} dy={18} textAnchor="middle" fill="#fff" fontSize={22} fontWeight={300} opacity={0.8}>
                 ...
                 <Area 
                    ...
                    strokeWidth={1.5} 
                    fill="url(#wavyGrad)" 
                    activeDot={{ r: 6, fill: '#fff', className: "shadow-[0_0_15px_rgba(255,255,255,1)]" }}
                    dot={({ cx, cy, index }) => {
                       if (index === 3) {
                         return (
                           <g key={`dot-${index}`}>
                              <circle cx={cx || 0} cy={cy || 0} r={5} fill="#fff" className="shadow-[0_0_20px_rgba(255,255,255,1)] drop-shadow-[0_0_10px_white]" />
                              <line x1={cx || 0} y1={cy || 0} x2={cx || 0} y2={(cy || 0) + 80} stroke="#fff" strokeDasharray="2 2" opacity={0.4} />
                           </g>
                         );
                       }
                       return <circle key={`dot-${index}`} cx={cx || 0} cy={cy || 0} r={3} fill="#fff" opacity={0.3} />;
                    }}
                 />
              </AreaChart>
           </ResponsiveContainer>
